    ...
    <main className={styles.page}>
      ...
      <section className={styles.card}>
        <div className={styles.left}>
          ...
          <form className={styles.form} noValidate onSubmit={(event) => { event.preventDefault(); void props.onSubmit(); }}>
            ...
            <button className={styles.primaryButton} type="submit" disabled={props.isSubmitting}>{props.isSubmitting ? "UPDATING…" : "UPDATE PASSWORD"}</button>
          </form>
          {props.error ? <p className={styles.error} role="alert" aria-live="polite">{props.error}</p> : null}
          {props.successMessage ? <p className={styles.success} role="status">{props.successMessage} <Link href="/login">Sign in</Link></p> : null}
        </div>
